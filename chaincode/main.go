...
type order struct {
	...
	GoodsOwnerName    	string 	`json:"goodsOwnerName"`
...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response {
	...
	order := &order{"order","orderId", "fromAddress", "toAddress", "coal", 20, 4000,"WAIT_DRIVER_ACCEPT","goodsOwnerId","brokerId0","driverId",getTimeNow(), true, ChangeStateHistory}
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "initOrder" { //create a new order
...
func (t *SimpleChaincode) initFileHash(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// ==== Create marble object and marshal to JSON ====
	ObjectType := "fileHash"
	fileHash := &StringHash{ObjectType, fileId, orderId, dataUrl, shaResult, comment}
...
func (t *SimpleChaincode) queryOrdersByBroker(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	queryString := fmt.Sprintf("{\"selector\":{\"docType\":\"order\",\"broker\":\"%s\"}}", broker)
...
func (t *SimpleChaincode) queryOrdersWithPagination(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	queryString := args[0]
	//return type of ParseInt is int64
	pageSize, err := strconv.ParseInt(args[1], 10, 20)
	if err != nil {
		return shim.Error(err.Error())
	}
	bookmark := args[2]
